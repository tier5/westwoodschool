...
jQuery(document).ready(function($){
	...
	// change status on fetched data
		$('#evoics_events').on('click','.row',function(){
	...
	// import selected items
		$('#evoics_import_selected_items').on('click', function(){
			...
			form.find('#evoics_import_progress p.text i').html(selected_row_count);
	...
	// IMPORT selected events
		function import_event_item(table, index, total, form, failed){
			...
			$.ajax({
				beforeSend: function(){	
					form.find('#evoics_import_progress').show();	
				},
				type: 'POST',
				...
				success:function(data){
					...
					if(data.status=='success'){
						...
						form.find('#evoics_import_progress p.bar span').css('width', per+'%' );
						form.find('#evoics_import_progress p.text em').html(index);
						...
						if(index == total){
							// show results
							good = total - failed;
							resultsOBJ = form.find('#evoics_import_results .results');
							resultsOBJ.find('.good em').html(good);
							resultsOBJ.find('.bad em').html(failed);
							$('#evoics_import_results').fadeIn();

							$('#select_row_options').hide();
							$('#evoics_import_progress').fadeOut();
						}
					}else{
						failed++;
						form.find('#evoics_import_progress span.failed').show().find('em').html(failed);
					...
					// last item
					if(index == total){
						form.find('#evoics_import_progress p.text b').removeClass('loading');	
					}
